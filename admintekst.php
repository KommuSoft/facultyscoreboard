...
<?
	...
	$prefa = "t";
	$pref = "'$prefa'";
	echo '<tr><td><input type="text" class="input" id="t_id" width="10%" style="width:25px;" readonly value="--"></td><td><input type="text" class="input" id="t_tekst"></td><td><input type="button" value="Leeg" class="btn" onclick="empty('.$pref.')">&nbsp;&nbsp;&nbsp;<input type="button" id="'.$prefa.'_btn" class="btn btn-primary" value="Toevoegen" onclick="submitModAdd('."$basa,$pref".')"></td></tr>';
...
while($row = pg_fetch_row($rs)) {
	$id = $row[0];
	$tekst = $row[1];
	echo "<tr><td>$id</td><td>$tekst</td>$buttons0$id$buttons1$id$buttons2</tr>\n";
}
pg_close($con);
?>
</table>
